...
> - **Siempre usa ramas feature** para cada nueva funcionalidad. Nunca trabajes directamente en `main`.
> - **Configura `.gitignore` desde el inicio** del proyecto para evitar subir archivos sensibles o de compilación.
> - **Usa `git revert` en código compartido**, nunca `git reset`. `revert` es seguro porque no rompe el historial de otros.
> - **Aprende los conceptos, no solo los comandos.** Saber POR QUÉ usas `rebase` vs `merge` es más importante que memorizar la sintaxis.
> - **Practica en un repositorio temporal** antes de aplicar cambios arriesgados. Los errores son la mejor forma de aprender.
